...
class ODLClient(object):
    ...
    def create_vInterface(self, vInterfaceName, vBridgeName, vtnName,
                          **params):
        data = {
            "input": {
                "tenant-name": vtnName,
    ...
    def create_vtnPortMap(self, vtnPortMap, node, vInterfaceName, vBridgeName,
                          vtnName, **params):
        data = {
            "input": {
                "tenant-name": vtnName,
    ...
    def set_path_policy(self, vtn_path_cost, policy, default_cost=1, **params):
        ...
        for element in vtn_path_cost:
            data['input']['vtn-path-cost'].append({
                "port-desc":
                "openflow:" + element.split("-")[0].split("s")[1] + "," +
                element[-1] + "," + element,
                "cost":
                vtn_path_cost[element]
            })
